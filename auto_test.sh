...
# Test case 10
test_no_params() {
    panic_detected="false"  # Используем строку "false" как начальное значение
    ...
    if [ "$panic_detected" = "false" ]; then
        ...
        if [ $? -eq 1 ]; then
            panic_detected="true"  # Устанавливаем флаг, если паника обнаружена
        fi
    fi

    # Если паника была обнаружена, завершаем выполнение теста
...
test_generate_duplicate_pattern() {
    ...
    if [[ -z "$result" || "$result" =~ ^[[:space:]]*$ ]]; then
        print_fail "Test case 23: Expected non-empty output, but got empty or whitespace-only output."
        test_failed=true
        return
    fi
    ...
    if [ $exit_code -ne 0 ]; then
        # Используем verify_output для проверки, что вывод содержит ошибку или что-то ожидаемое
        verify_output "error" "$result" "23"
